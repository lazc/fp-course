...
map' ::
  ...
  -> List a
  -> List b
map' f xs = foldRight ( \x -> (:.) (f x) )  Nil xs
...
filter' ::
  ...
  -> List a
  -> List a
filter' p = foldRight (\h -> bool id (h:.) (p h)) Nil 
...
flatten ((x :. xs) :. ls) =  x :. (flatten (xs :. ls))
...
seqOptional ::
  List (Optional a)
  -> Optional (List a)
--seqOptional list = if filter (\x = x == Empty) /= Nil else 
