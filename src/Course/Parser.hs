...
--thing :: Input -> ParseResult a
--thing Nil = ErrorResult Failed 
--thing (h :. t) = Result t h
...
list1 ::
 ...
 -- p and then (call it x)
  --list p and then (call it y)
  --always prod (x:. y)
list1 p = flbindParser p (\x -> flbindParser (list p) (\y -> valueParser (x :. y) ) )

--list1 p  = lift2 (:.) p (list p)

--list1 p = lift2 (:.) <*> list -- spaceship is via ->t def
...
satisfy ::
  (Char -> Bool)
  -> Parser Char
satisfy p = bindParser (\x -> if p x then valueParser x else unexpectedCharParser x  ) character
...
natural ::
  Parser Int
natural = list1 digit >>= \k -> case read k of Empty -> failed
                                               Full h -> pure h
...
sequenceParser ::
  List (Parser a)
  -> Parser (List a)
sequenceParser l = foldRight (lift2 (:.)) (valueParser Nil) l
...
firstNameParser ::
  Parser Chars
firstNameParser =  upper >>= (\x -> (lift2 (:.)) (valueParser x) (list lower) )
...
-- for monad, can use left vals on right
{-
do u <- upper
   v <- thisMany 5 lower
   w <- list lower
...
phoneParser ::
  Parser Chars
-- phoneParser = digit >>= ( \c -> lift2 (:.) (valueParser c) (phoneBodyParser >>= (\l -> (is '#') ) ) )
phoneParser = 
  do u <- digit
     v <- phoneBodyParser
     w <- is '#'
...
(:.) <$>
  digit <*>
  phoneBodyParser <*  -- half space ship wha!
  is #
-}
...
instance Applicative Parser where
  ...
  (<*>) ::
    ...
    -> Parser a
    -> Parser b
  (<*>) f a = bindParser (\f' -> mapParser (\k -> f' k) a) f
